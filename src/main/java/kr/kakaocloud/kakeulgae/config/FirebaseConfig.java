...
class FirebaseConfig{
    ...
    @Bean
    public FirebaseApp firebaseApp() throws IOException {//FirebaseApp을 Bean으로 등록 (Bean으로 등록된 객체는 스프링 컨테이너에 의해 관리되며, DI를 통해 주입받을 수 있다.)
        ...
        return FirebaseApp.initializeApp(options);//FirebaseApp을 초기화한다. (FirebaseApp은 Firebase의 인증, 데이터베이스, 스토리지 등의 기능을 사용하기 위한 진입점)
    }

    @Bean
    public FirebaseAuth firebaseAuth() throws IOException {
        return FirebaseAuth.getInstance(firebaseApp()); //FirebaseApp을 통해 FirebaseAuth를 초기화한다. (FirebaseAuth는 Firebase Authentication 서비스를 사용하기 위한 진입점)
    }
