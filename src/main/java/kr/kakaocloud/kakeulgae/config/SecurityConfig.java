   ...
   private final UserCustomDetailsService userDetailsService; // 사용자 정보를 가져오는 서비스
    ...
    public SecurityFilterChain springSecurity(HttpSecurity http) throws Exception {
         return http.csrf(AbstractHttpConfigurer::disable)
            ...
            .sessionManagement(ss->ss.sessionCreationPolicy(SessionCreationPolicy.STATELESS)) // 세션을 사용하지 않음
            .authorizeHttpRequests(it -> it.requestMatchers(FIREBASE_TOKEN_FILTER_PERMITTED_PATTERNS.toArray(new String[0])).permitAll().anyRequest().authenticated()) // 필터의 예외 URL을 설정
            .addFilterBefore(new FirebaseTokenFilter(userDetailsService, firebaseTokenHelper), UsernamePasswordAuthenticationFilter.class) // addFilterBefore()는 특정 필터를 다른 필터 앞에 추가하는 메소드
            .exceptionHandling(it-> it.authenticationEntryPoint(customAuthenticationEntryPoint)).build();// 인증되지 않은 사용자가 보호된 리소스에 액세스하려고 할 때 호출됨
    }
}
