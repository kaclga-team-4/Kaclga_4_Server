...
public class CareerService {
    ...
    public List<Career> updateUserCareer(Member member, List<Long> careerIds) {
        List<Career> careers = careerRepository.findByIdIn(careerIds);
        List<CareerMemberRelation> careerMemberRelations = careerMemberRepository.findByMember(
            member);
        ...
        for (Career career : careers) {
            newCareerMemberRelations.add(CareerMemberRelation.createRelation(member, career));
        }
        careerMemberRepository.saveAll(newCareerMemberRelations);
